...
type PhilosophyStyle = {
  icon: LucideIcon;
  color: string;
  bg: string;
};

const philosophyStyles: PhilosophyStyle[] = [
  { icon: Zap, color: "text-accent-cyan", bg: "bg-accent-cyan/10" },
  { icon: Network, color: "text-accent-indigo", bg: "bg-accent-indigo/10" },
  { icon: Code2, color: "text-accent-violet", bg: "bg-accent-violet/10" },
];
    ...
    <section className="bg-surface-dark py-32 px-6 lg:px-12 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10 flex flex-col lg:flex-row gap-16 items-center">
        <div className="flex-1 space-y-12">
          ...
          >
            <h2 className="text-5xl md:text-6xl font-display font-bold text-ink leading-tight">
              {t.leadership.titleLine1} <br />
              <span className="text-accent-cyan italic">{t.leadership.titleEmphasis}</span>
            </h2>
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <div>
                <h4 className="text-xl font-bold text-ink mb-2">{t.leadership.mentorship.title}</h4>
                <p className="text-ink-light/70 font-medium">{t.leadership.mentorship.desc}</p>
              </div>
            </motion.div>
            ...
            >
              ...
              <div>
                <h4 className="text-xl font-bold text-ink mb-2">{t.leadership.standards.title}</h4>
                <p className="text-ink-light/70 font-medium">{t.leadership.standards.desc}</p>
              </div>
            </motion.div>
          </div>
        </div>
        ...
        <div className="flex-1 w-full grid gap-6">
          {philosophyStyles.map((style, i) => {
            const copy = t.leadership.philosophies[i];
            ...
            return (
              ...
              >
                ...
                <h3 className="text-2xl font-display font-bold text-ink mb-3">{copy.title}</h3>
                <p className="text-ink-light/70 font-medium leading-relaxed">{copy.desc}</p>
              </motion.div>
            );
          })}
        </div>
